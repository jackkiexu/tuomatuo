...
//初始化
$(function(){
	searchInit("${webRoot}/whiteKeyNumber/search.do");     
...
function searchInit(url){
	$('#dg').datagrid({
		...
		{field:'id',title:'${(cmodel.whiteNumberKey_id)!''}',sortable:true,
				formatter:function(value,row,index){return row.id;} 
			},
			{field:'number',title:'${(cmodel.whiteNumberKey_number)!''}',sortable:true,
				formatter:function(value,row,index){return row.number;} 
			},
			{field:'createUser',title:'${(cmodel.whiteNumberKey_createUser)!''}',sortable:true,
				formatter:function(value,row,index){return whiteKeyWord_createUser_map[row.createUser];}
			},
			{field:'createTime',title:'${(cmodel.whiteNumberKey_createTime)!''}',sortable:true,
				formatter:function(value,row,index){return row.createTime;} 
			},
			{field:'isDelete',title:'${(cmodel.whiteNumberKey_isDelete)!''}',sortable:true,
	...
	pager.pagination({
		...
		buttons:[{
		      iconCls:'icon-info',
		      text:'详细',
		      disabled:noPermis("whiteKeyNumber","view"),
		      handler:function(){
		 	     viewObj("${(cmodel.whiteKeyNumber)!''}","${webRoot}/whiteKeyNumber/view.do?id=","","500");
		      }
		},{
		      iconCls:'icon-delete',
		      text:'标记删除',
		      disabled:noPermis("whiteKeyNumber","numberMarkDelete"),
		      handler:function(){
		         dealUserStatus("${(cmodel.whiteKeyNumber)!''}标记删除","${webRoot}/whiteKeyNumber/numberMarkDelete.do?id=");
		      }
		},{
		      iconCls:'icon-add',
		      text:'添加',
		      disabled:noPermis("whiteKeyNumber","save"),
		      handler:function(){
		 	     newObj("${(cmodel.whiteKeyNumber)!''}白名单","${webRoot}/whiteKeyNumber/save.do");
		      }
		},{
		      iconCls:'icon-edit',
		      text:'状态还原',
		      disabled:noPermis("whiteKeyNumber","numberMarkNotDelete"),
		      handler:function(){
		           dealUserStatus("${(cmodel.whiteKeyNumber)!''}状态还原","${webRoot}/whiteKeyNumber/numberMarkNotDelete.do?id=");
		      }
		}]
	});
}
...
function validation(obj){
	...
	var isDelete = $("#isDelete").val();
		 if(isNull(number)){
			err = "${(cmodel.whiteNumberKey_number)!''}不能为空";
			errId = "number";
		}else 
		{
			//success
...
<div id="tb" style="padding:5px;height:auto">
	<form id="searchForm">
	  ...
	  ${(cmodel.whiteNumberKey_id)!''}:<input type="text" name="search.EQI_id"/>
	  ${(cmodel.whiteNumberKey_number)!''}:<input type="text" name="search.LIKES_number"/>
...
<div id="editObj" style="display:none;">
    ...
    <table class="formTable" width="100%">
        <tbody>
            <tr class="keyWord">
                <td>${(cmodel.whiteNumberKey_number)!''}</td><td><input type="text" id="number" name="number" value="" /></td>
            </tr>
             <tr class="createUser">
                <td>${(cmodel.whiteNumberKey_createUser)!''}</td><td><select id="createUser" name="createUser"/></td>
            </tr>
        </tbody>
    </table>
...
<div id="viewObj" style="display:none;">
    ...
    <table class="formTable" width="100%">
        <tbody>
            <tr>
                <td>${(cmodel.whiteNumberKey_id)!''}</td><td id="view_id"></td>
            </tr>
            <tr>
                <td>${(cmodel.whiteNumberKey_number)!''}</td><td id="view_keyWord"></td>
            </tr>
            <tr>
                <td>${(cmodel.whiteNumberKey_createUser)!''}</td><td id="view_createUser"></td>
            </tr>
            <tr>
                <td>${(cmodel.whiteNumberKey_createTime)!''}</td><td id="view_createTime"></td>
            </tr>
            <tr>
                <td>${(cmodel.whiteNumberKey_isDelete)!''}</td><td id="view_isDelete"></td>
            </tr>
        </tbody>
    </table>
    </form>
