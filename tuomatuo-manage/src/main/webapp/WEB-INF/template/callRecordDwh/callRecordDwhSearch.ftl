...
$(function(){
	...
	searchInit("${webRoot}/callRecordDwh/search.do");         
...
function searchInit(url){
	$('#dg').datagrid({
		...
		idField:'id', //主键字段
		queryParams:{"search.GED_startTime":time2,"search_startTime":time2}, //查询条件
	...
	pager.pagination({
		...
		buttons:[{
		      iconCls:'icon-info',
		      text:'详细',
		      disabled:noPermis("callRecordDwh","view"),
		      handler:function(){
		 	     viewObj("${(cmodel.callRecord)!''}","${webRoot}/callRecordDwh/view.do?id=","","400");
		      }
		},{
		      iconCls:'icon-edit',
		      text:'添加黑名单',
		      disabled:noPermis("callRecordDwh","addBlock"),
		      handler:function(){
		          dealUserStatus("${(cmodel.callRecord)!''}添加黑名单","${webRoot}/callRecordDwh/addBlock.do?id=");
		      }
		},{
		      iconCls:'icon-tack',
		      text:'添加全局黑名单',
		      disabled:noPermis("callRecordDwh","addGlobalBlack"),
		      handler:function(){
		          dealUserStatus("${(cmodel.callRecord)!''}添加黑名单","${webRoot}/callRecordDwh/addGlobalBlack.do?id=");
		      }
		}]
	});
}
...
<div id="tb" style="padding:5px;height:auto">
	<form id="searchForm">
      ...
      <input type="button" onclick="do_search();" value="查询"/><input type="reset" value="清空" onclick="searchReset();"/>
      </div>
  	</form>         
</div>
