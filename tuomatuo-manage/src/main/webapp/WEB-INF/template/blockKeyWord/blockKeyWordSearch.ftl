...
//初始化
$(function(){
	searchInit("${webRoot}/blockKeyWord/search.do");     
...
function searchInit(url){
	$('#dg').datagrid({
		...
		{field:'id',title:'${(cmodel.blockKeyWord_id)!''}',sortable:true,
				formatter:function(value,row,index){return row.id;} 
			},
			{field:'keyWord',title:'${(cmodel.blockKeyWord_keyWord)!''}',sortable:true,
				formatter:function(value,row,index){return row.keyWord;} 
			},
			{field:'createUser',title:'${(cmodel.blockKeyWord_createUser)!''}',sortable:true,
				formatter:function(value,row,index){return whiteKeyWord_createUser_map[row.createUser];}
			},
			{field:'createTime',title:'${(cmodel.blockKeyWord_createTime)!''}',sortable:true,
				formatter:function(value,row,index){return row.createTime;} 
			},
			{field:'isDelete',title:'${(cmodel.blockKeyWord_isDelete)!''}',sortable:true,
	...
	pager.pagination({
		...
		buttons:[{
		      iconCls:'icon-info',
		      text:'详细',
		      disabled:noPermis("blockKeyWord","view"),
		      handler:function(){
		 	     viewObj("${(cmodel.blockKeyWord)!''}","${webRoot}/blockKeyWord/view.do?id=","","500");
		      }
		},{
		      iconCls:'icon-remove',
		      text:'标记删除',
		      disabled:noPermis("blockKeyWord","markDelete"),
		      handler:function(){
		         dealUserStatus("${(cmodel.blockKeyWord)!''}标记删除","${webRoot}/blockKeyWord/markDelete.do?id=");
		      }
		},{
		      iconCls:'icon-add',
		      text:'添加',
		      disabled:noPermis("blockKeyWord","save"),
		      handler:function(){
		 	     newObj("${(cmodel.blockKeyWord)!''}白名单","${webRoot}/blockKeyWord/save.do");
		      }
		},{
		      iconCls:'icon-edit',
		      text:'状态还原',
		      disabled:noPermis("blockKeyWord","markNotDelete"),
		      handler:function(){
		          dealUserStatus("${(cmodel.blockKeyWord)!''}状态","${webRoot}/blockKeyWord/markNotDelete.do?id=");
		      }
		}]
	});
}
...
<div id="tb" style="padding:5px;height:auto">
	<form id="searchForm">
	  ...
	  ${(cmodel.blockKeyWord_id)!''}:<input type="text" name="search.EQI_id"/>
	  ${(cmodel.blockKeyWord_keyWord)!''}:<input type="text" name="search.LIKES_keyWord"/>
...
<div id="editObj" style="display:none;">
    ...
    <table class="formTable" width="100%">
        <tbody>
            <tr class="keyWord">
                <td>${(cmodel.blockKeyWord_keyWord)!''}</td><td><input type="text" id="keyWord" name="keyWord" value="" /></td>
            </tr>
             <tr class="createUser">
                <td>${(cmodel.blockKeyWord_createUser)!''}</td><td><select id="createUser" name="createUser"/></td>
            </tr>
        </tbody>
    </table>
...
<div id="viewObj" style="display:none;">
    ...
    <table class="formTable" width="100%">
        <tbody>
            <tr>
                <td>${(cmodel.blockKeyWord_id)!''}</td><td id="view_id"></td>
            </tr>
            <tr>
                <td>${(cmodel.blockKeyWord_keyWord)!''}</td><td id="view_keyWord"></td>
            </tr>
            <tr>
                <td>${(cmodel.blockKeyWord_createUser)!''}</td><td id="view_createUser"></td>
            </tr>
            <tr>
                <td>${(cmodel.blockKeyWord_createTime)!''}</td><td id="view_createTime"></td>
            </tr>
            <tr>
                <td>${(cmodel.blockKeyWord_isDelete)!''}</td><td id="view_isDelete"></td>
            </tr>
        </tbody>
    </table>
    </form>
