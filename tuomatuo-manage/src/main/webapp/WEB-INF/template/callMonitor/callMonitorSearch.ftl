...
//初始化
$(function(){
	searchInit("${webRoot}/callMonitor/search.do");
	
	$("#callState").html("<option value='' selected>请选择</option>");
	$("#callState").append(getSelectList(callMonitorOfAll_callState_map,"1")); 
})
...
function searchInit(url){
	$('#dg').datagrid({
		...
		columns:[[
			...
			{field:'userId',title:'${(cmodel.callRepeatVO_userId)!''}',sortable:true,
				formatter:function(value,row,index){return row.userId;} 
			},
			{field:'mobileNumber',title:'${(cmodel.callRepeatVO_mobileNumber)!''}',sortable:true,
				formatter:function(value,row,index){return row.mobileNumber;} 
			},
			{field:'repeatCount',title:'${(cmodel.userCallRepeatVO_repeatCount)!''}',sortable:true,
	...
	pager.pagination({
		...
		buttons:[{
		      iconCls:'icon-lock',
		      text:'锁定帐号',
		      disabled:noPermis("callMonitor","lock"),
		      handler:function(){
		 	     dealUserStatus("${(cmodel.callMonitor)!''}锁定","${webRoot}/callMonitor/lock.do?id=");
		      }
		},{
		      iconCls:'icon-unlock',
		      text:'解锁帐号',
		      disabled:noPermis("callMonitor","unlock"),
		      handler:function(){
		          dealUserStatus("${(cmodel.callMonitor)!''}解锁","${webRoot}/callMonitor/unlock.do?id=");
		      }
		},{
		     iconCls:'icon-add',
		      text:'添加黑名单',
		      disabled:noPermis("callMonitor","unlock"),
		      handler:function(){
		          addMobileToBlockList("${(cmodel.callMonitor)!''}添加入黑名单","${webRoot}/callMonitor/addMobileToBlockList.do?mobile=");
		      }
		}]
	});
}
...
function addMobileToBlockList(){
	...
	url = url + mobileNumber + "&&isPlusChar=" + mobileNumber.indexOf("+");
	if(!confirm("你确认把号码" + mobileNumber + "加入黑名单")){return false;}
	$.ajax({
        type: "post",
        dataType: "json",
        url: url,
...
<div id="tb" style="padding:5px;height:auto">
	<form id="searchForm">
	   	<div>
		    ...
		    ${(cmodel.callRepeatVO_userId)!''}:<input type="text" name="userId"/>
		    ${(cmodel.callRepeatVO_mobileNumber)!''}:<input type="text" name="mobileNumber"/><br/>
			  结束时间: <input class="easyui-datetimebox" name="endTime" style="width:80px" id="endTime">
		  	${(cmodel.callRepeatVO_repeatCount)!''}:<input type="text" name="repeatCount"/>
		  	查询类型:<select id="callState"  name="callState"></select>
