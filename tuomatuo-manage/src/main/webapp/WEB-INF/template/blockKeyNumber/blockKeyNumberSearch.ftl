...
//初始化
$(function(){
	searchInit("${webRoot}/blockKeyNumber/search.do");     
...
function searchInit(url){
	$('#dg').datagrid({
		...
		{field:'id',title:'${(cmodel.blockNumberKey_id)!''}',sortable:true,
				formatter:function(value,row,index){return row.id;} 
			},
			{field:'number',title:'${(cmodel.blockNumberKey_number)!''}',sortable:true,
				formatter:function(value,row,index){return row.number;} 
			},
			{field:'createUser',title:'${(cmodel.blockNumberKey_createUser)!''}',sortable:true,
				formatter:function(value,row,index){return whiteKeyWord_createUser_map[row.createUser];}
			},
			{field:'createTime',title:'${(cmodel.blockNumberKey_createTime)!''}',sortable:true,
				formatter:function(value,row,index){return row.createTime;} 
			},
			{field:'isDelete',title:'${(cmodel.blockNumberKey_isDelete)!''}',sortable:true,
	...
	pager.pagination({
		...
		buttons:[{
		      iconCls:'icon-info',
		      text:'详细',
		      disabled:noPermis("blockKeyNumber","view"),
		      handler:function(){
		 	     viewObj("${(cmodel.blockKeyNumber)!''}","${webRoot}/blockKeyNumber/view.do?id=","","500");
		      }
		},{
		      iconCls:'icon-delete',
		      text:'标记删除',
		      disabled:noPermis("blockKeyNumber","numberMarkDelete"),
		      handler:function(){
		         dealUserStatus("${(cmodel.blockKeyNumber)!''}标记删除","${webRoot}/blockKeyNumber/numberMarkDelete.do?id=");
		      }
		},{
		      iconCls:'icon-add',
		      text:'添加',
		      disabled:noPermis("blockKeyNumber","save"),
		      handler:function(){
		 	     newObj("${(cmodel.blockKeyNumber)!''}白名单","${webRoot}/blockKeyNumber/save.do");
		      }
		},{
		      iconCls:'icon-edit',
		      text:'状态还原',
		      disabled:noPermis("blockKeyNumber","numberMarkNotDelete"),
		      handler:function(){
		           dealUserStatus("${(cmodel.blockKeyNumber)!''}状态还原","${webRoot}/blockKeyNumber/numberMarkNotDelete.do?id=");
		      }
		}]
	});
}
...
function validation(obj){
	...
	var isDelete = $("#isDelete").val();
		 if(isNull(number)){
			err = "${(cmodel.blockNumberKey_number)!''}不能为空";
			errId = "number";
		}else{
			//success
			return true;
...
<div id="tb" style="padding:5px;height:auto">
	<form id="searchForm">
	  ...
	  ${(cmodel.blockNumberKey_id)!''}:<input type="text" name="search.EQI_id"/>
	  ${(cmodel.blockNumberKey_number)!''}:<input type="text" name="search.LIKES_number"/>
...
<div id="editObj" style="display:none;">
    ...
    <table class="formTable" width="100%">
        <tbody>
            <tr class="keyWord">
                <td>${(cmodel.blockNumberKey_number)!''}</td><td><input type="text" id="number" name="number" value="" /></td>
            </tr>
             <tr class="createUser">
                <td>${(cmodel.blockNumberKey_createUser)!''}</td><td><select id="createUser" name="createUser"/></td>
            </tr>
        </tbody>
    </table>
...
<div id="viewObj" style="display:none;">
    ...
    <table class="formTable" width="100%">
        <tbody>
            <tr>
                <td>${(cmodel.blockNumberKey_id)!''}</td><td id="view_id"></td>
            </tr>
            <tr>
                <td>${(cmodel.blockNumberKey_number)!''}</td><td id="view_keyWord"></td>
            </tr>
            <tr>
                <td>${(cmodel.blockNumberKey_createUser)!''}</td><td id="view_createUser"></td>
            </tr>
            <tr>
                <td>${(cmodel.blockNumberKey_createTime)!''}</td><td id="view_createTime"></td>
            </tr>
            <tr>
                <td>${(cmodel.blockNumberKey_isDelete)!''}</td><td id="view_isDelete"></td>
            </tr>
        </tbody>
    </table>
    </form>
