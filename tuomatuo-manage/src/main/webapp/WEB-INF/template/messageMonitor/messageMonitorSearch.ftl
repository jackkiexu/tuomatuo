...
//初始化
$(function(){
	searchInit("${webRoot}/messageMonitor/search.do");
	
	$("#callState").html("<option value='' selected>请选择</option>");
	$("#callState").append(getSelectList(msg_monitor_map,"1")); 
})
...
function searchInit(url){
	$('#dg').datagrid({
		...
		columns:[[
			...
			{field:'userId',title:'${(cmodel.messageRepeatVO_userId)!''}',sortable:true,
				formatter:function(value,row,index){return row.userId;} 
			},
			{field:'mobileNumber',title:'${(cmodel.messageRepeatVO_mobileNumber)!''}',sortable:true,
				formatter:function(value,row,index){return row.mobileNumber;} 
			},
			{field:'repeatCount',title:'${(cmodel.messageRepeatVO_repeatCount)!''}',sortable:true,
	...
	pager.pagination({
		...
		buttons:[{
		      iconCls:'icon-lock',
		      text:'锁定帐号',
		      disabled:noPermis("messageMonitor","lock"),
		      handler:function(){
		 	     dealUserStatus("${(cmodel.messageMonitor)!''}锁定","${webRoot}/messageMonitor/lock.do?id=");
		      }
		},{
		      iconCls:'icon-unlock',
		      text:'解锁帐号',
		      disabled:noPermis("messageMonitor","unlock"),
		      handler:function(){
		          dealUserStatus("${(cmodel.messageMonitor)!''}解锁","${webRoot}/messageMonitor/unlock.do?id=");
		      }
		}]
	});
}
...
<div id="tb" style="padding:5px;height:auto">
	<form id="searchForm">
	   	<div>
		    ...
		    ${(cmodel.messageRepeatVO_userId)!''}:<input type="text" name="userId"/>
		    ${(cmodel.messageRepeatVO_mobileNumber)!''}:<input type="text" name="mobileNumber"/><br/>
			  结束时间: <input class="easyui-datetimebox" name="endTime" style="width:80px" id="endTime">
		  	${(cmodel.messageRepeatVO_repeatCount)!''}:<input type="text" name="repeatCount"/>
