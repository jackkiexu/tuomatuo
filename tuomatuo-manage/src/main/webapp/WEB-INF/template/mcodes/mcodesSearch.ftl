...
//初始化
$(function(){
	searchInit("${webRoot}/mcodes/search.do");
...
function searchInit(url){
	$('#dg').datagrid({
		...
		idField:'id', //主键字段
		queryParams:{'search.EQI_ctype':'1'}, //查询条件
		...
		columns:[[
			...
			{field:'userId',title:'M码卡密',sortable:true,
				formatter:function(value,row,index){return row.codes;} 
			},
			{field:'fromNumber',title:'M码状态',sortable:true,
	...
	pager.pagination({
		...
		buttons:[{
		      iconCls:'icon-add',
		      text:'添加',
		      disabled:noPermis("mcodes","save"),
		      handler:function(){
		      	randomCodes();	//生成优惠码
		 	    newObj("M码","${webRoot}/mcodes/save.do");
		      }
		}]
	});
}
...
function randomCodes(){
	$.ajax({
        type: "post",
        dataType: "json",
        url: "${webRoot}/mcodes/randomCodes.do",
        data: {length:"6"},
...
<div id="tb" style="padding:5px;height:auto">
	<form id="searchForm">
		  ...
		  M码卡密:<input type="text" name="search.LIKES_codes"/>
		  ${(cmodel.codes_userId)!''}:<input type="text" name="search.LIKES_userId"/>
		  M码状态:<select id="search_status" name="search.EQI_status"></select>
...
<div id="editObj" style="display:none;">
    ...
    <table class="formTable" width="100%" style="padding:3px">
        <tbody>
            <tr>
                <td width="120px">M码卡密</td><td><input type="text" id="new_codes" readOnly="true" name="codes" value=""/>   <input type="button" onclick="randomCodes()" value="生成M码" /></td>
            </tr>
            <tr>
