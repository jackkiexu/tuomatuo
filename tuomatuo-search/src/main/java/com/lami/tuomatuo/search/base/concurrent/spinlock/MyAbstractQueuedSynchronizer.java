...
public abstract class MyAbstractQueuedSynchronizer extends AbstractOwnableSynchronizer
    ...
    private static final Logger logger = Logger.getLogger(MyAbstractQueuedSynchronizer.class);
    ...
    static final class Node {
        ...
        public String toString() {
            String ss =  "Node : status :" + waitStatus ;
            if(thread != null){
                ss = ss + ", ThreadName:" +thread.getName();
            }else{
                ss = ss + "Thread is null";
            }
            return ss;
        }
    ...
    private Node enq(final Node node) {
        logger.info(Thread.currentThread().getName() + " " + "enq : " + node);
        for (;;) {
            Node t = tail;
    ...
    private Node addWaiter(Node mode) {
        logger.info(Thread.currentThread().getName() + " " +"addWaiter : " + mode );
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        logger.info(Thread.currentThread().getName() + " " +"shouldParkAfterFailedAcquire : " + pred + ", node : " + node + ", ThreadName:" + Thread.currentThread().getName());
    ...
    private static void selfInterrupt() {
        logger.info("selfInterrupt, ThreadName:" + Thread.currentThread().getName());
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (;;) { // 当 node 的前一个节点是真正的head时不断的自旋
                       // 为什么是 node的前一个节点而不是node本身(因为一开始AQS里面的头节点是个空的节点)
                final Node p = node.predecessor();
                logger.info(Thread.currentThread().getName() + " " +"node.predecessor : " + p );
                logger.info(Thread.currentThread().getName() + " " +"node.predecessor head :" + head );
                if (p == head && tryAcquire(arg)) {
                    logger.info(Thread.currentThread().getName() + " " +"p == head && tryAcquire(arg) success" );
                    setHead(node);
                    p.next = null; // help GC
                    failed = false;
                    return interrupted;
                }
                // 判断node是否需要park或interrupt
                if (shouldParkAfterFailedAcquire(p, node) && parkAndCheckInterrupt())
                    logger.info(Thread.currentThread().getName() + " interrupted");
                    interrupted = true;
            }
        } finally {
            logger.info(Thread.currentThread().getName() + " " +"acquireQueued : " + node + ", arg : " + 1 + ", failed : " + failed + ", ThreadName:" + Thread.currentThread().getName());
            if (failed)
                cancelAcquire(node);
        }
    }
    ...
    public final boolean release(int arg) {
        logger.info("release arg: " + arg + Thread.currentThread().getName());
        if (tryRelease(arg)) {
            Node h = head;
            logger.info("head : " + head + Thread.currentThread().getName());
    ...
    public final boolean hasQueuedPredecessors() {
        ...
        // 因为空队列时我们需要插入一个 空的Node, 所以 用了h.next.thread 来进行比较
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
         ...
         * Implements {@link MyAbstractQueuedSynchronizer#hasWaiters}.
         *
         ...
         * this condition.
         * Implements {@link MyAbstractQueuedSynchronizer#getWaitQueueLength}.
         *
         * @return the estimated number of waiting threads
         * @throws IllegalMonitorStateException if {@link #isHeldExclusively}
         *         returns {@code false}
         ...
         * Implements {@link MyAbstractQueuedSynchronizer#getWaitingThreads}.
         *
         * @return the collection of threads
         * @throws IllegalMonitorStateException if {@link #isHeldExclusively}
         *         returns {@code false}
