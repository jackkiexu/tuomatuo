...
/**
 * Channel interface
 * A channel is a representation of a group nodes all participating in some sort of
 ...
 * 1. send messages
 * 2. receive message (by registering a <code>ChannelListener</code>)
 * 3. get all members of the group <code>getMember()</code>
 * 4. receive notifications of members added and members disappeared by registering a
...
public interface Channel {
     ...
     * Note:
     *     In order for the membership broadcaster to transmit the correct information, it has be started after the replication receover
     *
     ...
     * SND_RX_SEQ - stops te replication receiver
     *
     ...
     *                    is that a membership change can occur and at that time the application is uncertain what group the messages
     *                    actually got sent to
     * @param msg Serializable - the message to send, has to be serializable, or <code>ByteMessage</code> to send a purg byte array
     * @param options int - sender options, see class documentation for each interceptor that is configured in order to
     *                to trigger interceptor
    ...
    /**
     * Add a channel listener, this a callback object when messages are received
     * <br>
     ...
     * recent data. Some implementatons might serialize and send
