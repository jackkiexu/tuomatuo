...
import com.lami.tuomatuo.mq.netty.util.internal.ThrowableUtil;
...
public class DefaultPromise<V> extends AbstractFuture<V> implements Promise<V> {
    ...
    private static AtomicReferenceFieldUpdater<DefaultPromise, Object> RESULT_UPDATER;
    ...
    private static final CauseHolder CANCELLATION_CAUSE_HOLDER = new CauseHolder(
            ThrowableUtil.unknowStackTrace(new CancellationException(), DefaultPromise.class, "cancel(.....)")
    );
    ...
    static {
        ...
        RESULT_UPDATER = updater == null ? AtomicReferenceFieldUpdater.newUpdater(DefaultPromise.class, Object.class, "result") : updater;
    }
    ...
    /**
     * One or more listeners, Can be a {@link GenericFutureListener} or a {@link io.netty.util.concurrent.DefaultFutureListeners}
     * if {@code null}, it means either 1) no listener were added yet or 2) all listeners were notified
     *
    ...
    /**
     * Threading - synchrounized(this). We must prevent concurrent notification and FIFO listener notification if the
     * executor changes
     */
    private boolean notifyingListners;
    ...
    public boolean await(long timeoutMillis, TimeUnit unit) throws InterruptedException{
        return await0(unit.toNanos(timeoutMillis), true);
    }
     ...
     */
    protected static void notifyListener(EventExecutor executor,
                                         final Future<?> future, final GenericFutureListener<?> listener){
        ObjectUtil.checkNotNull(executor, "eventExecutor");
        ObjectUtil.checkNotNull(future, "future");
        ObjectUtil.checkNotNull(listener, "listener");
        notifyListenerWithStackOverFlowProtection(executor, future, listener);
    }
    ...
    /**
     * The logic in this method should be identical to {@link #notifyingListners} but
     * cannot share code because the listener(s) cannot be cached for an instance of {@link DefaultPromise} since the
     * listener(s) may be changed and is protected by a synchrounized operation
     */
    ...
    private void notifyListenersNow(){
        ...
        synchronized (this){
            ...
            if(notifyingListners || this.listeners == null){
                return;
            }
            notifyingListners = true;
    ...
    private static void notifyListener0(Future future, GenericFutureListener l){
        try {
            ...
            e.printStackTrace();
            logger.info("An exception was thrown by " + l.getClass().getName() + ".operationComplete()", e);
        }
    }
    ...
    private void removeListener0(GenericFutureListener<? extends Future<? super V>> listener){
        if(listeners instanceof DefaultFutureListeners){
            ((DefaultFutureListeners)listener).remove(listener);
    ...
    /**
     * Notfy all progressive listeners
     * <p>
     *     No attempt is make to ensure notification order if multiple calls are make to this method before
     *     the original invocation completes
     * </p>
     *
     * This will do an interation over all listeners to get all of type {@link GenericProgressiveFutureListener}s
     *
        ...
        if(executor.inEventLoop()){
            ...
            if(listeners instanceof GenericProgressiveFutureListener[]){
                ...
                safeExecute(executor, new Runnable() {
                    @Override
                    public void run() {
                        notifyProgressiveListener0(self, array, progress, total);
                    }
                });
            }else{
    ...
    private synchronized Object progressiveListeners(){
        ...
        if(listeners instanceof  DefaultFutureListeners){
            // Copy DefutureListeners into an array of listeners
    ...
    private static void notifyProgressiveListener0(ProgressiveFuture<?> future,
                                                   GenericProgressiveFutureListener<?>[] listeners, long progress, long total){
    ...
    private static void notifyProgressiveListener0(ProgressiveFuture future,
        ...
        try {
            ...
            e.printStackTrace();
            logger.warn("An exception was thrown by " + l.getClass().getName() + ".operationProgressed()", e);
        }
    }
    ...
    private static void safeExecute(EventExecutor executor, Runnable task){
        try {
            ...
            e.printStackTrace();
            rejectedExecutionLogger.error("Failed to submit a listener notification task. Event loop shut down?", e);
        }
    }
