...
public class CommandArgs<K, V> {
    ...
    public CommandArgs<K, V> add(String s) { return write(s); }

    public CommandArgs<K, V> add(long n) { return write(Long.toString(n)); }

    public CommandArgs<K, V> add(double n) { return write(Double.toString(n));}
    ...
    private CommandArgs<K, V> write(String arg){
        ...
        if(buffer.remaining() < length) realloc(buffer.capacity() * 2);
