 ...
 * </pre></blockquote>
 */
public class FileTxnLog implements org.apache.zookeeper.server.persistence.TxnLog {
    ...
    static {
        LOG = LoggerFactory.getLogger(org.apache.zookeeper.server.persistence.FileTxnLog.class);
            ...
            throws IOException
    {
        if (hdr != null) {
            ...
            padFile(fos);
            byte[] buf = org.apache.zookeeper.server.persistence.Util.marshallTxnEntry(hdr, txn);
            ...
            org.apache.zookeeper.server.persistence.Util.writeTxnBytes(oa, buf);
    ...
    private void padFile(FileOutputStream out) throws IOException {
        currentSize = org.apache.zookeeper.server.persistence.Util.padLogFile(out, currentSize, preAllocSize);
    }
    ...
    public static File[] getLogFiles(File[] logDirList,long snapshotZxid) {
        List<File> files = org.apache.zookeeper.server.persistence.Util.sortDataDir(logDirList, "log", true);
        long logZxid = 0;
        // Find the log file that starts before or at the same time as the
        // zxid of the snapshot
        for (File f : files) {
            long fzxid = org.apache.zookeeper.server.persistence.Util.getZxidFromName(f.getName(), "log");
    ...
    public long getLastLoggedZxid() {
        ...
        long maxLog=files.length>0?
                org.apache.zookeeper.server.persistence.Util.getZxidFromName(files[files.length-1].getName(),"log"):-1;
        ...
        try {
            org.apache.zookeeper.server.persistence.FileTxnLog txn = new org.apache.zookeeper.server.persistence.FileTxnLog(logDir);
            itr = txn.read(maxLog);
            while (true) {
                if(!itr.next())
                    break;
     ...
     */
    public static class FileTxnIterator implements org.apache.zookeeper.server.persistence.TxnLog.TxnIterator {
        File logDir;
        long zxid;
        TxnHeader hdr;
        Record record;
        ...
        void init() throws IOException {
            ...
            List<File> files = org.apache.zookeeper.server.persistence.Util.sortDataDir(org.apache.zookeeper.server.persistence.FileTxnLog.getLogFiles(logDir.listFiles(), 0), "log", false);
            for (File f: files) {
                if (org.apache.zookeeper.server.persistence.Util.getZxidFromName(f.getName(), "log") >= zxid) {
                    storedFiles.add(f);
                }
                // add the last logfile that is less than the zxid
                else if (org.apache.zookeeper.server.persistence.Util.getZxidFromName(f.getName(), "log") < zxid) {
                    storedFiles.add(f);
                    break;
                }
            }
        ...
        protected void inStreamCreated(InputArchive ia, InputStream is)
            ...
            if (header.getMagic() != org.apache.zookeeper.server.persistence.FileTxnLog.TXNLOG_MAGIC) {
                throw new IOException("Transaction log: " + this.logFile + " has invalid magic number "
                        + header.getMagic()
                        + " != " + org.apache.zookeeper.server.persistence.FileTxnLog.TXNLOG_MAGIC);
            }
        }
